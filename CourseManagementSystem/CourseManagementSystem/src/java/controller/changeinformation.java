...
public class changeinformation extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String newu = request.getParameter("newusername");
        String newp = request.getParameter("newpassword");
        String newn = request.getParameter("newfullname");
        String newe = request.getParameter("newemail");
        accountDAO ad = new accountDAO();
        String messenger = ad.updateInfo(u.getUserid(), newu, newp, newn, newe, u.getAvatar());
        users u1 = (users) session.getAttribute("user");
        session.setAttribute("messenger", messenger);
        if ( messenger.equals("Update successfully !")){
